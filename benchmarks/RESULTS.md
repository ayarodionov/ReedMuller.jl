...
and the start of RM(2,10)) and its continuation
[`compare_large_resume_m10.jl`](compare_large_resume_m10.jl) (the rest
of RM(2,10)) — see [Reproducing this run](#reproducing-this-run)
below for why it's split across two scripts and how to redo it in one
piece.
...
- Decoders: `ReedDecoder`, `ChaseDecoder(ReedDecoder(); t=4)`,
  ...
  `:pairs`/PmCr ensembles). See [ALGORITHMS.md](../ALGORITHMS.md) for
...
13-decoder data is in the tables below). A hollow marker denotes zero
...
The two logs were then combined by hand into the tables above. If you
hit the same thing, `sysctl vm.swapusage` / `memory_pressure` (macOS)
tell you whether it's memory pressure before you assume it's a bug.
